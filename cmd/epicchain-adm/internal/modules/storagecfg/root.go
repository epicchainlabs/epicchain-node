...
import (
	...
	netutil "github.com/epicchainlabs/epicchain-node/pkg/network"
...
func storageConfig(cmd *cobra.Command, args []string) {
	...
	historyPath := filepath.Join(os.TempDir(), "epicchain-adm.history")
	...
	for {
		...
		if err != nil {
			cmd.Println("Incorrect address format. See https://github.com/epicchainlabs/epicchain-node/blob/master/pkg/network/address.go for details.")
			continue
		}
	...
	cmd.Println("Node is ready for work! Run `epicchain-node -config " + outPath + "`")
}
